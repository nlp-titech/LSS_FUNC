...
parser.add_argument("--dense_model_path")
parser.add_argument("--splade_model_path")
...
model = splade.Splade(args.splade_model_path)
model.eval()
...
splade_rerank_results = retriever.rerank(corpus, queries, results, top_k=100)

model = DRES(models.SentenceBERT(args.dense_model_path), batch_size=128)
...
rerank_results = splade_rerank_results
for qid in rerank_results:
    for did in rerank_results[qid]:
        if did in dense_rerank_results[qid]:
            rerank_results[qid][did] += dense_rerank_results[qid][did]
