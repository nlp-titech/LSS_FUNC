...
from lss_func.indexing.codecs.residual_embeddings import ResidualEmbeddings


@dataclasses.dataclass
class CodecConfig:
    total_visible_gpus: int = 0
    dim: int = 768
    nbits: int = 2
...
class ResidualCodec:
    ...
    def __init__(self, config, centroids, avg_residual=None, bucket_cutoffs=None, bucket_weights=None):
        self.total_visible_gpus = config.total_visible_gpus
    ...
    def load(cls, index_dir):
        ...
        centroids_path = os.path.join(index_dir, "centroids.pt")
        avgresidual_path = os.path.join(index_dir, "avg_residual.pt")
        buckets_path = os.path.join(index_dir, "buckets.pt")
    ...
    def save(self, index_path):
        ...
        config_path = os.path.join(index_path, "index_config.json")
        ...
        with open(config_path, "w") as fOut:
            config = CodecConfig(total_visible_gpus=self.total_visible_gpus, dim=self.dim, nbits=self.nbits)
            json.dump(dataclasses.asdict(config), fOut)
    ...
    def decompress(self, compressed_embs: Embeddings):
        ...
        for codes_, residuals_ in zip(codes.split(1 << 15), residuals.split(1 << 15)):
            # Original Colbert's code use gpu implementation.
            # See https://github.com/stanford-futuredata/ColBERT/blob/7d52265a4de953dd05270437800aba583e2281cd/colbert/indexing/codecs/residual.py#L249-L260
